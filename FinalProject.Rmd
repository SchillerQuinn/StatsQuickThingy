...
bigHitters.effects <- effect %>%
  group_by(OPID) %>%
  tally() %>%
  filter(n >=5)

effect <- effect %>%
  filter(OPID %in% bigHitters.effects$OPID)

strAOS  <- strikes %>% 
  drop_na(AOS)

#find airlines that have more than 5 hits
bigHitters.strAOS <- strAOS %>%
  group_by(OPID) %>%
  tally() %>%
  filter(n >=5)

strAOSBig <- strAOS %>%
  filter(OPID %in% bigHitters.strAOS$OPID)
```
...
strikes %>% 
  ...
  scale_x_log10()+
  geom_histogram()+
  ggtitle("Number of Hits by Airline")
